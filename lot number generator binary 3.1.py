...
#creates multiple of one label by increasing the pallet number until desired amount reached
def iteration(colorString, boardString, lineBin, prodDateBin, palletNum, lotNum):
    i = 0
    while i == 0:
        howMany = input("How many more pallets of this type do you want to create? (1,2,3,4...):" )
        howMany = int(howMany)
        if howMany >= 5:
            print("Too many pallets to create in one day, try again")
        if howMany <= 4:
            printToFile(colorString, boardString, lineBin, prodDateBin, palletNum, lotNum)
...
while r == 1:
   ...
   #get data by calling functions
    ...
    #create iterated files
#startIterations = input('Do you want to iterate this lot? Y/N:')
#startIterations = startIterations.lower()
#if startIterations == 'y':
    iterationQ = input('How many iterations of this type do you want? (0,1,2,3...): ')
    iterationQ = int(iterationQ)
    o = 1
